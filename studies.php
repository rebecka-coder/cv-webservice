...
//Switch-metod för att använda antingen, get, post, update eller delete
 switch($method) {
     case "GET":
     //Kod för GET
     $response = $r->getStudies();
     if(sizeof($response) > 0) {
         http_response_code(200); //Ok
     } else {
         http_response_code(404); //Not found
         $response = array("message" => "Inga studier hittade."); //Felmeddelande
     }
   
     break;
     case "POST":
    //Kod för POST
        if ($input['university'] == "" || $input['studyName'] == "" || $input['studyDate'] == "") {
        http_response_code(500); //Server error
        $response = array("message" => "Alla fält måste fyllas i!");
        } else { 
        $c->addStudies($input['university'], $input['studyName'], $input['studyDate']); 
        http_response_code(201); //Skapad
        $response = array("message" => "Utbildning/kurs skapad.");
        }  

    break;
    case "DELETE":
    //Kod för DELETE
    if($d->delStudies($input['id'])) {
        http_response_code(200); //Ok
        $response = array("message" => "Utbildning/kurs raderad.");
    } else {
        http_response_code(500); //Server error
        $response = array("message" => "Error radera utbildning/kurs."); //Felmeddelande
    }
    break;
    case "PUT":
    //Kod för PUT
    if($u->updateStudies($input['university'], $input['studyName'], $input['studyDate'], $input['id'])) {
        http_response_code(200); //Ok
        $response = array("message" => "Utbildning/kurs uppdaterad.");
    } else {
        http_response_code(500); //Server error
        $response = array("message" => "Error uppdatera utbildning/kurs."); //Felmeddelande
    }
    break;
 }
